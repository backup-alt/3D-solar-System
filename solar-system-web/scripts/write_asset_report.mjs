...
const specs = [
  ...
  ['neptune', ['Uranus', 'Clouds'], ['Uranus Textur.jpg', 'clouds.jpg'], 'This source names its main mesh Uranus. Neither retained mesh has a UV map, so spherical UVs are generated on the web copies. The outer Clouds shell is retained; two inner cloud/atmosphere shells are omitted because they sit inside the surface and provide no visible exterior detail. Diffuse/Mix/Displacement nodes become textured PBR plus alpha cloud layer.'],
];
const rows = [];
...
for (const [key, selected, textures] of specs) {
  ...
  const sourceTextures = textures.map(name => {
